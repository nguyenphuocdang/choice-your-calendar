...
get name() { return this.createScheduleForm.get('name'); }
get brief() { return this.createScheduleForm.get('brief'); }
get timeData() { return this.createScheduleForm.get('timeData'); }
...
addNewSchedule(form: UntypedFormGroup) 
{
  ...
  this.calendarService.create(requestData).subscribe(
    (response: any) =>
    {
      if (response == null)
      {
      ...
      else 
      {
        ...
        if (response.statusCode!= null)
        {
          ...
          {
            this.toastrService.success('Create schedule successfully', 'Success 200');
            ...
            this.dialogRef.close({'isCreatedSuccessfully': isCreatedSuccessfully, 'newSchedule': schedule});
            return ;
          }
          if (response.statusCode == 400)
          {
            this.toastrService.error(response.data[0].errorMessage, 'Error 400');
            return ;
          }
        }
...
onStartTimeChange(value: any, indexDate: number, indexTimeOfDate: number)
{
  ...
  {
    this.toastrService.error('End Time must be greater than Start Time', 'TIME ERROR');
  }
  ...
  if (indexTimeOfDate != 0)
  {
    for (let i = 0; i < this.timeArray[indexDate].length-1; i++)
    {
      ...
      if (inputEndTime == '')
      {
        if (timeArrayElement.startTime < inputStartTime && inputStartTime < timeArrayElement.endTime)
        {
          this.toastrService.error('The event number #' + `${indexTimeOfDate+1}` + ' overlaps on ' + `${timeArrayElement.weekday}`, 'TIME OVERLAP ERROR');
          return ;
        }
      }
      else
      {
        if (timeArrayElement.startTime < inputEndTime && inputStartTime < timeArrayElement.endTime)
        {
          this.toastrService.error('The event number #' + `${indexTimeOfDate+1}` + ' overlaps on ' + `${timeArrayElement.weekday}`, 'TIME OVERLAP ERROR');
          return ;
        }
...
copyEvents(index: number)
{
  for(let i = 0; i < this.checkListCopy.length; i++)
  {
    ...
    {
      if (this.timeArray[i][0].startTime == '' && this.timeArray[i][0].endTime == '') 
      {
