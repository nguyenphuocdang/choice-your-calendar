import { Component, OnInit, forwardRef, Optional, ViewChild  } from '@angular/core';
...
export class CalendarComponent implements OnInit {
  displayedColumns: string[] = ['ID', 'name','brief','view','status','startDate','endDate'];
  ...
  {
    this.calendarService.getListSchedules().subscribe(
      (response: any) =>
      {
  ...
  {
    this.calendarService.getListSchedulesPagination(page,size).subscribe(
      (response: any) =>
      {
        ...
        if (response.pagination != null && response.result != null)
        {
          for (let i = 0; i < response.result.length; i++)
          {
            ...
            schedule.active = (schedulesElement.active == true) ? 'In Active' : 'No Longer In Use';
            listSchedule.push(schedule);
          }
          this.dataArray = listSchedule;
          this.dataSource = new MatTableDataSource<Schedule>(this.dataArray);
  ...
  viewScheduleDetail(elementId: any)
  {
    ...
    this.calendarService.getScheduleDetail(elementId).subscribe(
      (response: any) =>
      {
        if (response != null)
        {
          ...
          const dialogRef = this.dialog.open(DetailScheduleComponent, {data: INITIAL_EVENTS});
        }
      }
  )
