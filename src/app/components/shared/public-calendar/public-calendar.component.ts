...
export class PublicCalendarComponent implements OnInit {
    ...
    private localStorageService: LocalStorageService) 
  {
    ...
    this.duration = this.Activatedroute.snapshot.queryParamMap.get('duration') ?? '';
   }
  ...
  async ngOnInit(): Promise<void> 
  {
    ...
    //   };
    // }
    await this._getPublicFreeTimeSchedules(this.bookingEvent.partnerPathMapping, this.fromDate, this.toDate, true);
  }
  ...
  async _getPublicFreeTimeSchedules(pathMapping: string, fromDate: string, toDate: string, freeScheduleFlag: boolean)
  {
    try 
    {
      const response: any = await this.calendarService.getPublicFreeTimeUsingPromise(pathMapping, fromDate,toDate,freeScheduleFlag);
          ...
          for (let i = 0; i < (<any>response).data.scheduleDatas.length;i++)
          {
            ...
            if (schedule.timeDatas.length > 0)
            {
              for (let j = 0 ; j < schedule.timeDatas.length; j ++)
              {
                ...
                freeSlots.weekday = freeSlots.weekday.split('-').reverse().join('-');
  ...
  _addEvent()
  {
    ...
    this.popupService.$isConfirm.subscribe(
      (isConfirm) =>
      {
        if (this.popupService.isConfirm)
        {
          ...
          let _convertToDateStart: Date = new Date(_reverseDate + 'T' + requestBody.startTime+ ':00');
          let _convertToDateEnd: Date = new Date(_reverseDate + 'T' + requestBody.endTime+ ':00');
  ...
  async getComparedCalendar()
  {
    ...
    const responseComparedCalendar : any = await this.calendarService.getComparedCalendarPromise(this.bookingEvent.partnerPathMapping,this.fromDate,this.toDate, true);    
    if (responseComparedCalendar.statusCode === 200 && (<any>responseComparedCalendar).data.scheduleDatas.length > 0)
    {

      this.freeScheduleId = (<any>responseComparedCalendar).data.scheduleId;
      this.freeTimeScheduleSlots.listTimeWorkingDatas = [];

      for (let i = 0; i < (<any>responseComparedCalendar).data.scheduleDatas.length;i++)
      {
  ...
  onSelectFreeTimeSlot(slot: any)
  {
    ...
    let _convertToDate: Date = new Date(tempDate + 'T' + slot.startTime+ ':00');
  ...
  getAllTimeSlots()
  {
    this._getPublicFreeTimeSchedules(this.bookingEvent.partnerPathMapping, this.fromDate, this.toDate, true);
  }
