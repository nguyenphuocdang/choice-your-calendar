...
import { FreeTimeScheduleSlots, ListTimeWorkingDatas, ShareScheduleRequest } from 'src/app/_models/schedule';
...
export class InvitePartnersComponent implements OnInit {
  ...
  defaultToDate: string = new Date(this.defaultDate.getFullYear(), this.defaultDate.getMonth(), this.defaultDate.getDate()+12).toISOString().replace(/T.*$/, '');
  constructor(
  ...
  _sharedCalendar()
  {
    ...
    let fromDate: string = new Date(this.defaultDate.getFullYear(), this.defaultDate.getMonth(), this.defaultDate.getDate()+2).toISOString().replace(/T.*$/, ''); 
    let toDate: string = new Date(this.defaultDate.getFullYear(), this.defaultDate.getMonth(), this.defaultDate.getDate()+12).toISOString().replace(/T.*$/, '');

    let _convertSelectedDateStart: Date = new Date(fromDate);
    let _convertSelectedDateEnd: Date = new Date(toDate);  
    let _convertSelectedStartUTC: string = _convertSelectedDateStart.toISOString();
