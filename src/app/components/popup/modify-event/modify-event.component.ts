...
export class ModifyEventComponent implements OnInit {
  ...
  async modifyEvent() {
    ...
    try {
      const responseDetailCalendar =
        await this.CalendarService.getScheduleDetailPromise(this.scheduleId);
      if (responseDetailCalendar != null) {
        let modifiedSchedule: ModifyScheduleRequest = {
          ...
          listTimeWorkingDatas: (<any>responseDetailCalendar).data
            .listTimeWorkings,
        };
